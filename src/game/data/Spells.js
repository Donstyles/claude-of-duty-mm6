...
function spell(def) {
  ...
  return {
    ...
    /**
     * What `magnitude` is a quantity *of*: `{ ac }`, `{ resist:[…] }`,
     * `{ stats:[…] }`, or `{ rider, type }` for weapon enchantments. Without it
     * a buff is a number nothing reads — `Character.refresh()` sums `statBonus`
     * and `acBonus` off every buff, and until this field existed no spell in
     * the book set either, so Stone Skin and Bless were mechanically identical
     * to casting nothing at all.
     */
    affects: def.affects ? Object.freeze(def.affects) : null,
...
const FIRE = [
  ...
  spell({
    ...
    affects: { rider: 'damage', type: 'fire' },
    tags: ['enchant'],
  ...
  spell({
    ...
    target: 'party', delivery: 'aura',
    duration: hours(1), magnitude: amount(4, 0.6, { normal: 0, expert: 2, master: 4, grandmaster: 6 }),
    condition: 'hasted',
    affects: { stats: ['speed'] },
    tags: ['buff'],
...
const AIR = [
  ...
  spell({
    ...
    target: 'party', delivery: 'aura',
    duration: hours(1), magnitude: amount(2, 0.4),
    affects: { ac: true },
    tags: ['buff', 'defence'],
...
const LIGHT = [
  ...
  spell({
    ...
    affects: { stats: ['might', 'intellect', 'personality', 'endurance', 'accuracy', 'speed', 'luck'] },
    tags: ['buff'],
  ...
  spell({
    ...
    target: 'party', delivery: 'aura',
    duration: hours(1), magnitude: amount(0, 3),
    affects: { resist: ['fire', 'air', 'water', 'earth', 'mind', 'body'] },
    tags: ['buff', 'resistance'],
...
const DARK = [
  ...
  spell({
    ...
    target: 'item', delivery: 'enchant',
    duration: hours(1), magnitude: amount(0, 1),
    affects: { rider: 'lifesteal', type: 'dark' },
    tags: ['enchant'],
